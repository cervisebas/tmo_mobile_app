import {
  UserBookStatusList,
  UserStatus,
} from '~/api/interfaces/UserBookStatus';
...
export class DatabaseService {
  ...
  public async getAllSavedBooks(): Promise<BookInfoInterface[]> {
    const books = await this.db.select().from(BookInfoModel);
  ...
  public async getAllUserChapterHistory() {
    try {
      const find_all = await this.db
        ...
        .innerJoin(
          BookInfoModel,
          eq(BookInfoModel.id, BookUserChapterHistoryModel.id_bookinfo),
        )
        .innerJoin(
          BookChapterModel,
          eq(BookChapterModel.id, BookUserChapterHistoryModel.id_chapter),
        )
        .innerJoin(
          BookChapterOptionModel,
          eq(BookChapterOptionModel.id_chapter, BookChapterModel.id),
        );

      const data: UserChapterHistoryInterface[] = [];

      for (const item of find_all) {
        const find_book = data.findIndex(
          (v) => v.chapter.id === item[DatabaseTableName.BOOK_CHAPTERS].id,
        );
          ...
          const temp: Partial<UserChapterHistoryInterface> = {
            ...
            chapter: {
              ...
              chapter_number: extractNumberChapter(
                item[DatabaseTableName.BOOK_CHAPTERS].name,
              ),
              options: [
                {
  ...
  public async getCurrentHistoryBook(id_bookinfo: number) {
    const info = await this.db
      ...
      .where(eq(BookUserChapterBookHistoryModel.id_bookinfo, id_bookinfo));
  ...
  public async getDatabaseHistoryChapter<T extends number | number[]>(
    id_chapter: T,
  ): Promise<
    T extends number
      ? boolean
  ...
  public async getUserStatusBooks(
    status: UserBookStatus | 'all',
    ...
    const books = await this.db
      ...
      .innerJoin(
        BookInfoModel,
        eq(BookInfoModel.id, BookUserStatusByBookInfoModel.id_bookinfo),
      )
      .where(
        status === 'all'
  ...
  public async getUserStatusCount() {
    ...
    for (const status of user_status) {
      if (status.marked) {
        data[status.status].quantity = String(
          Number(data[status.status].quantity) + 1,
        );
      }
    }
  ...
  public async removeAllUserChapterHistory() {
    await this.db.delete(BookUserChapterHistoryModel);
  }

  public async removeUserChapterHistory(id: number) {
    await this.db
      .delete(BookUserChapterHistoryModel)
      .where(eq(BookUserChapterHistoryModel.id, id));
  }
  ...
  public async setDatabaseHistoryChapter(
    ...
    const chapter_set_list: Parameters<
      typeof this.dbSave.saveChaptersHistory
    >[0] = [];
    ...
    for (const item of chapterList) {
      if (chapter.chapter_number >= item.chapter_number) {
        chapter_set_list.push({
          ...
          //status: true,
          status:
            chapter.chapter_number === item.chapter_number ? status : true,
        });
      } else {
        chapter_set_list.push({
          id_chapter: item.id!,
  ...
  public async setMarkUserBookStatus(
    id_bookinfo: number,
  ...
  public async removeDatabaseChapter(id_chapter: number) {
    this.db.transaction(async (tx) => {
      await tx
        ...
        .where(eq(BookChapterOptionModel.id_chapter, id_chapter));

      await tx
        .delete(BookChapterModel)
        .where(eq(BookChapterModel.id, id_chapter));
    });
  }
}
